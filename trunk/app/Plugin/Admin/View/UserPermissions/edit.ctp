    ...
    <?php $data['allowed'] = (in_array(strtolower($roleName), $adminRoleName)) ? 1 : $data['allowed']; ?>
        <tr>
            ...
            <td align="right">
                ...
                ?>
                <a href="javascript:;;" id="<?php echo 'permission-' . $acoPath[count($acoPath)-1]['Aco']['id'] . '-' . $data['id']; ?>" onclick="<?php if ($roleName != 'Administrator'): ?>acos.toggle('<?php echo $this->Html->url('/admin/user_permissions/toggle');?>',<?php echo $acoPath[count($acoPath)-1]['Aco']['id']; ?>, <?php echo $data['id']; ?>);return false;<?php endif; ?>">
                    <?php echo $this->Html->image('/admin/img/icons/allow-' . $data['allowed'] . '.png'); ?>
                </a>
                <?php
                }
                ?>
